...
namespace App\Registrar\Repository;
...
/**
 * @method CertificationRecord|null find($id, $lockMode = null, $lockVersion = null)
 * @method CertificationRecord|null findOneBy(array $criteria, array $orderBy = null)
 * @method CertificationRecord[]    findAll()
 * @method CertificationRecord[]    findBy(array $criteria, array $orderBy = null, $limit = null, $offset = null)
 */
class CertificationRecordRepository extends ServiceEntityRepository
{
    private $connection;
    
    public function __construct(RegistryInterface $registry)
    {
        parent::__construct($registry, CertificationRecord::class);
...
//        $sql = "SELECT *, @row_number:=@row_number+1 AS row_number FROM `" . $params['table'] . "`";
    ...
    public function save(CertificationRecord $certificationRecord): void {
        $this->getEntityManager()->persist($certificationRecord);
        $this->getEntityManager()->flush();
        return;
    }

    public function update(CertificationRecord $certificationRecord): void {
        $this->getEntityManager()->flush();
        return;
    }

    public function remove(CertificationRecord $certificationRecord): void {
        $this->getEntityManager()->remove($certificationRecord);
        $this->getEntityManager()->flush();
        return;
    }
}
