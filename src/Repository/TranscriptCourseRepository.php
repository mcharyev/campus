...
/**
 * @method TranscriptCourse|null find($id, $lockMode = null, $lockVersion = null)
 * @method TranscriptCourse|null findOneBy(array $criteria, array $orderBy = null)
 * @method TranscriptCourse[]    findAll()
 * @method TranscriptCourse[]    findBy(array $criteria, array $orderBy = null, $limit = null, $offset = null)
 */
class TranscriptCourseRepository extends ServiceEntityRepository {

    private $connection;

    public function __construct(RegistryInterface $registry) {
        parent::__construct($registry, TranscriptCourse::class);
    ...
    public function getCourseGrades(array $params = []): array {
        $sql = "SELECT * FROM `transcript_course` WHERE letter_code = '" . $params['courseCode'] . "' AND year = '" . $params['year'] . "' AND student_id = '" . $params['studentId'] . "';";
        $stmt = $this->connection->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll();
    }

    public function save(TranscriptCourse $transcriptCourse): void {
        $this->getEntityManager()->persist($transcriptCourse);
        $this->getEntityManager()->flush();
        return;
    }

    public function update(TranscriptCourse $transcriptCourse): void {
        $this->getEntityManager()->flush();
        return;
    }

    public function remove(TranscriptCourse $transcriptCourse): void {
        $this->getEntityManager()->remove($transcriptCourse);
        $this->getEntityManager()->flush();
        return;
    }
